...
1. Open the project with **Unity 6.5** matching `ProjectSettings/ProjectVersion.txt` (or allow Unity to upgrade the patch and commit the new version if the team agrees).
2. Package Manager resolves `com.unity.ml-agents` **4.1.0** and Test Framework without errors.
...
6. Open `Assets/EvoLife/Scenes/EvoLifeDemo.unity`, press **Play**, and confirm the research-diorama presentation (see [PRESENTATION.md](PRESENTATION.md)):
   - overview shows grassland basin + forest (N/NW) + wetland (W/SW) + rocky (S/SE) with **irregular** visual edges (not painted circles)
   - forest uses Kenney tree meshes with clearings / non-colliding trunks; grassland stays open
   - wetland shows shallow irregular ponds + reeds/lilies; water triggers do not trap creatures
   - rocky area uses Kenney rock clusters (no cliffs / maze)
   - herbivores are Kenney deer, predators are Kenney foxes; facing (+Z) is obvious while moving
   - edible Kenney bushes distinguishable from decorative grass tufts
   - no decorative colliders; creature roots keep CapsuleColliders
   - no duplicate founders / duplicate plant nodes after one Play start
   - day/night stays readable at night; drought lushness / wildfire cues do not mutate ResourceManager
   - UI can attach to `Agent10_CameraRigHook` / `Agent10_UiCanvasHook` without a second enabled camera
   - `Bootstrap.unity` remains a separate lightweight training-oriented scene
...
9. Commit any Unity-generated `.meta` files so GUIDs are shared (scenes/prefabs break without them).
